function events = sta_lta_2(time,wave,edp,type)
%
...
abs_v = abs(v);;
iat = 0;
%disp(['DEBUG: length of data = ',num2str(l_v)])
%  **** Deb added above

%% Event Detection Parameters
%l_sta = edp(1)*Fs;   % STA window size
%l_lta = edp(2)*Fs;   % LTA window size
l_sta = edp(1);   % STA window size
l_lta = edp(2);   % LTA window size
...
skip_int = edp(5);% Skip ahead after end of event (seconds)
min_dur = edp(6); % Any triggers longer than min_dur become events
...
%% Calculate STA/LTA Ratio
   sta_to_lta = sta/lta;
   iat = iat + 1;
   sta_to_lta_save(iat) = sta_to_lta;

%% Trigger on? (Y/N)
   if sta_to_lta > th_on
      %disp('DEBUG: Threshold exceeded');
...
%figure(11)
%clf
%plot(sta_to_lta_save,'co-','Color',[0.2 0.6 1.0]);
%axis tight
%%mm = median(sta_to_lta_save);
%mmm = mean(sta_to_lta_save);
%mmm_std = std(sta_to_lta_save);
%grid on
%xx = get(gca,'Xlim');
%hold on
%%plot(xx,[mm mm],'b--','LineWidth',2);
%plot(xx,[mmm mmm],'r-','LineWidth',2,'Color',[0.8 0.8 0.8]);
%plot(xx,[(mmm-mmm_std) (mmm-mmm_std)],'r--','LineWidth',2,'Color',[0.2 0.2 0.2]);
%plot(xx,[(mmm+mmm_std) (mmm+mmm_std)],'r--','LineWidth',2,'Color',[0.2 0.2 0.2]);
%legend('Data','Mean','Mean-std','Mean+std','Location','NorthWest');
%title(['Debug: mean(sta/lta)=',num2str(mmm,2)]);
%xlabel('Index')
%ylabel('STA/LTA Ratio')
%set(gca,'FontSize',12)
%print stalta_rat.png -dpng
% *** deb added above
...
switch lower(type)
   case {'ssd'}
      events = trig_array;
   case {'sst'}
  %disp(['Debug = ',num2str(max(trig_array))]);
