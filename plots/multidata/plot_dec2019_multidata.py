...
## Output figure path:
multidataplot_path_pdf = '/Users/vjs/turbidites/observational/figs/pdf/multidataDec2019.pdf'
multidataplot_path_png = '/Users/vjs/turbidites/observational/figs/png/multidataDec2019.png'
...
events2019 = pd.read_csv(events2019path)
## Get datetime column:
events2019['FormattedTime'] = pd.to_datetime(events2019.Time.astype('datetime64[ms]'))
...
ylabel_ordered = ['Oxygen','Chlorophyll ()','Sea Water \nTemperature (C)','Turbidity (NTU)']
...
for i_axis,i_dataframe,i_column,i_ylabel in zip(multiaxes,dataframe_ordered,column_ordered,ylabel_ordered):
    ...
    ## scatter plot:
    i_axis.plot(i_dataframe.datetime,i_dataframe[i_column],linewidth=0.9,color='#056e6c')
    ...
    i_axis.vlines(events2019.loc[36:37].FormattedTime.values,i_ymin,i_ymax,linewidth=1,linestyle='--',color='#cc8904')

    ## Set ylimits:
    i_axis.set_ylim([i_ymin,i_ymax])
    
    ## Axis formatter:
    i_axis.yaxis.set_tick_params(labelsize=14)
...
i_axis.xaxis.set_major_formatter(mdates.DateFormatter('%m-%d-%H'))
i_axis.xaxis.set_tick_params(labelsize=14,rotation=0)  
